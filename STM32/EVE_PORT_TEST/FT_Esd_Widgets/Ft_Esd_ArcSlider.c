...
void Ft_Esd_ArcSlider_Update_TouchTracker_NoWrap(Ft_Esd_ArcSlider *context)
{
	...
	auto max = context->ESD_Partial_Ring.MaxAngle(context->ESD_Partial_Ring.Owner);
	auto min = 0;
	while (step > 180) 
	{
		step -= 360;
	...
	auto adjustAngle = (!!context->Clockwise(context->Owner)) ? (currAngle + step) : (currAngle - step);
	if (adjustAngle > max) 
	{
		adjustAngle = max;
	...
	//if (step != 0) 	//ESD_print(" Start: %d, Pre: %d, Track: %d, StartTrack: %d, ", context->StartAngle, context->PreAngle, context->TrackAngle, context->StartTrackAngle);
	...
	//if (step != 0) 	//ESD_print(" step: %d, new: %d, adjust: %d ", step, newAngle, adjustAngle, context->PreAngle);
}
...
void Ft_Esd_ArcSlider_Update_TouchTracker(Ft_Esd_ArcSlider *context, ft_int16_t tag)
{
	...
	if (tagId == tag)	
	{
		...
		//ESD_print(" angle: %d,[%d] tagRd: %d ", angle, tmp >> (16 + 4), tagId);
		...
		else
		{
			...
			if (context->ClampedNewAngle < 0) context->ClampedNewAngle += 360;
			else if (context->ClampedNewAngle > 360) context->ClampedNewAngle -= 360;
			context->StartAngle = context->ClampedNewAngle;			
			//ESD_print(" >> Start: %d, track: %d\n", context->StartAngle, context->StartTrackAngle);
		}
	}
}

...
void Ft_Esd_CircleLine_Render_Func(Ft_Esd_CircleLine *context, int x, int y, int radius, int border, ft_argb32_t color)
{
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_MASK(1,1,1,0));	
...
void Ft_Esd_CircleLine_SetDSTAlpha()
{
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BLEND_FUNC(ONE, ONE_MINUS_SRC_ALPHA));
}

void Ft_Esd_CircleLine_ClearDSTAlpha()
{
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BLEND_FUNC(ZERO, ONE_MINUS_SRC_ALPHA));
}
...
{
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX_FORMAT(4));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BEGIN(FTPOINTS));
	int iR = radius * 16 + 0.5f;	
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, POINT_SIZE(iR));
	int iX = (x * 16) + 0.5f;
	int iY = (y * 16) + 0.5f;
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX2F(iX, iY));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, END());
}
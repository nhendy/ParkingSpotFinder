...
void Ft_Esd_ArcLine_Render_Func(Ft_Esd_ArcLine *context, int x, int y, int radius, int border, ft_argb32_t color, ft_bool_t isClockwise, int origin, int value)
{
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, SAVE_CONTEXT()); // A

	Ft_Esd_Rect16 scissor = {
		.X = context->Widget.GlobalX - 1, .Y = context->Widget.GlobalY - 1,
		.Width = context->Widget.GlobalWidth + 2, .Height = context->Widget.GlobalHeight + 2
	};
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, CLEAR_COLOR_RGB(0, 0, 0));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, CLEAR_COLOR_A(0));
	//Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, CLEAR(1, 1, 1));		

	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_MASK(0, 0, 0, 1));
	...
	if (!isClockwise) originSector = 3 - originSector;
	if (isClockwise)
	{
	...
	if (!isClockwise) endSector = 3 - (endSector % 4);
	if ((value < 270) ||
		(value >= 270 && originSector != endSector))
	{
		if (isClockwise)
		{
	...
	if (value < 360 && value > 0)
	{
		if (!context->ShowStartPoint)
		{
			...
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_A(255));
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_RGB(r, g, b));

			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX_FORMAT(4));
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, LINE_WIDTH(16));
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BEGIN(LINES));
			int _x = x0;
			int _y = y0;
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX2F(_x, _y));
			_x = x1;
			_y = y1;
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX2F(_x, _y));
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_RGB(r, g, b));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_A(a));
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, RESTORE_CONTEXT()); // A


	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, SAVE_CONTEXT()); // B
	...
	if (context->ShowStartPoint)
	{
		...
		{
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_RGB(r, g, b));
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_A(185));
	...
	if (context->ShowEndPoint)
	{
		...
		if (context->ShowPointShadow)
		{
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_RGB(r >> 1, g >> 1, b >> 1));
			Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_A(125));
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, RESTORE_CONTEXT()); // B



	if (endAngle > 360 && (endAngle % 90) > 0)
	{
		Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, SAVE_CONTEXT()); // C
		...
		if (!isClockwise) sector = 3 - sector;
		...
		Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, RESTORE_CONTEXT()); // C
		...
		if (value < 360 && value > 0)
		{
			if (!context->ShowEndPoint)
			{
				...
				Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, SAVE_CONTEXT()); // D
				...
				Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, RESTORE_CONTEXT()); // D
			}
		}
...
void Ft_Esd_ArcLine_Draw_SectorArea(int x, int y, int radius, ft_bool_t isClockwise, int origin, int value)
{
	while (origin < 0) origin += 360;
	while (origin > 360) origin -= 360;
...
{
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX_FORMAT(4));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BITMAP_LAYOUT(L8, 1, 1));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BITMAP_SIZE(NEAREST, REPEAT, REPEAT, side, side));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BEGIN(BITMAPS));
	//Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, LINE_WIDTH(16));
	for (int i = 0; i < count; i++)
	{
		Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX2F((x[i] * 16), (y[i] * 16)));
	}
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, END());
}
...
void Ft_Esd_ArcLine_Draw_Line(int x0, int y0, int x1, int y1)
{
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, LINE_WIDTH(16));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BEGIN(LINES));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX2F((x0 * 16), (y0 * 16)));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX2F((x1 * 16), (y1 * 16)));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, END());
	*/
}
...
void Ft_Esd_ArcLine_Draw_Arc_Left(ft_float_t angle, int x, int y, int radius, ft_argb32_t color, ft_bool_t isClockwise, ft_float_t refAngle)
{
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, POINT_SIZE(radius * 16));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX_FORMAT(4));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BEGIN(EDGE_STRIP_L));
	//x = x + 1;
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX2F((x * 16), (y * 16)));
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, END());
	//printf("\n", x, y);
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, COLOR_RGB(0, 0, 0));
...
void Ft_Esd_ArcLine_Draw_Arc_Right(ft_float_t angle, int x, int y, int radius, ft_argb32_t color, ft_bool_t isClockwise, ft_float_t refAngle)
{
	...
	if (isClockwise && angle == 0) return;
	if (!isClockwise && angle == 360) return;
...
{ // Refactored. Angle is in the direction
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, POINT_SIZE(radius * 16));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX_FORMAT(4));
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, BEGIN(EDGE_STRIP_A));
	//x = (isClockwise) ? x -1: x;
	//y = y + 1;
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, VERTEX2F((x * 16), (y * 16)));

...
{
	Ft_Esd_RadioGroup *rg = context->RadioGroup(context->Parent);
	if (rg) rg->Checked = context;
	else Ft_Esd_RadioButton_GlobalChecked = context;
}
...
{
	Ft_Esd_RadioGroup *rg = context->RadioGroup(context->Parent);
	if (rg) return rg->Checked == context;
	else return Ft_Esd_RadioButton_GlobalChecked == context;
}

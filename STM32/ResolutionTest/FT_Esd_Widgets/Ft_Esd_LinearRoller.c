...
ft_argb32_t Ft_Esd_LinearRoller_GetGradientColor(ft_argb32_t colorA, ft_argb32_t colorB, ft_float_t scale)
{
	...
	if (scale < 0) return colorA;
	else if (scale > 1) return colorB;
...
void Ft_Esd_LinearRoller_Draw(Ft_Esd_LinearRoller *context)
{
	...
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, SCISSOR_XY( max(context->Widget.GlobalX, 0), max(context->Widget.GlobalY, 0))); 
	Ft_Gpu_Hal_WrCmd32(Ft_Esd_Host, SCISSOR_SIZE( context->Widget.GlobalWidth, context->Widget.GlobalHeight));

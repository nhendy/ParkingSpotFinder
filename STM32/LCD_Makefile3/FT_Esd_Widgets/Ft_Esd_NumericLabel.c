...
void Ft_Esd_NumericLabel_ComputePosition(Ft_Esd_NumericLabel *context)
{
	...
	//ESD_print(" isRight: %d  isCenter: %d ", isRight, isCenter);
	context->Option = 0;
...
void Ft_Esd_NumericLabel_UpdateNumberString(Ft_Esd_NumericLabel *context)
{
	...
	if (context->EnablePadding(owner) && context->NDigit > 0)
	{
		...
		{	//  Zero Padding
			//ESD_print((isHex) ? "0x%0*X\n" : "%0*d\n", context->NDigit, value);
			ESD_sprint(context->Number, (isHex) ? "0x%0*X" : "%0*d", context->NDigit, value);
		}
		else
		{	// Space Padding
			//ESD_print((isHex) ? "0x% *X\n": "% *d\n", context->NDigit, value);
			ESD_sprint(context->Number, (isHex) ? "0x% *X" : "% *d", context->NDigit, value);
		}		
	}
	else
	{
		//ESD_print((isHex) ? "0x%X\n" : "%d\n", value);
		ESD_sprint(context->Number, (isHex) ? "0x%X" : "%d", value);
	}	
}

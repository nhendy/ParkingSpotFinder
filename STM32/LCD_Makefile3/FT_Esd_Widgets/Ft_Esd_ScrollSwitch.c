...
void Load_NewPage(Ft_Esd_ScrollSwitch *context)
{
	if (context->State & _Loading) 
	{
		...
		if (context->Spinner(context->Owner)) Ft_Esd_Spinner_Popup();
...
void Ft_Esd_ScrollSwitch_Recover_Position(Ft_Esd_ScrollSwitch *context)
{
	if (!(context->State & _Recovering)) return;
	if (context->Touching) return;
...
void Ft_Esd_ScrollSwitch_Touch_Up(Ft_Esd_ScrollSwitch *context)
{
	if (context->State & _Changing) return;
	context->Overflow = 0;
	...
	if (dx > minDx && context->Index < context->Count)
	{
		...
		if (!context->Looping && index == context->Count) index --;
	}
	else if (dx < -minDx && context->Index > -1)
	{
		...
		if (!context->Looping && index == -1) index ++;	
	}	
	...
	{		
		...
		if (removingWidget == context->Current) removingWidget = NULL;
		...
		if (context->Looping)
		{
			if (context->NewIndex < 0) context->NewIndex += context->Count;
			if (context->NewIndex >= context->Count) context->NewIndex %= context->Count;	 		
		}
		else
		{
			if (context->NewIndex < 0) context->NewIndex = index;
			if (context->NewIndex >= context->Count) context->NewIndex = index;
		}
...
void Ft_Esd_ScrollSwitch_Update_Widget(Ft_Esd_ScrollSwitch *context, Ft_Esd_Widget* newWidget)
{
	...
	if (!(context->State & _Loading) || (context->State & _Shifting)) return;

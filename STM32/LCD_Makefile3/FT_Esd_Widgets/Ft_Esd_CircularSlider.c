...
void Ft_Esd_CircularSlider_Update_TouchTracker_NoWrap(Ft_Esd_CircularSlider *context)
{
	...
	if 	(step == 0 && !context->IsReady) return;
	...
	if (newValue > context->MaxValue) newValue = context->MaxValue;
	if (newValue < 0) newValue = 0;	
		
	if (newValue != context->Value)
	{
		//if (step != 0) 	//ESD_print(" Start: %d, Track: %d, StartTrack: %d, ", context->StartAngle, context->TrackAngle, context->StartTrackAngle);
	...
	//if (step != 0) 	//ESD_print(" step: %d, new: %d, adjust: %d ", step, newAngle, adjustAngle);

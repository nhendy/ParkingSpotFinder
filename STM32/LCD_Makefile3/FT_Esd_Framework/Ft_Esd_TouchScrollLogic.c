...
void Ft_Esd_TouchScrollLogic_TouchUpdate_Then(Ft_Esd_TouchScrollLogic *context)
{
	...
	//ESD_print("Switch: X %d, Y %d\n",context->EnableXScroll(owner), context->EnableYScroll(owner));
	if (context->EnableXScroll(owner)) HandleXTouchChange(context);
	if (context->EnableYScroll(owner)) HandleYTouchChange(context);
}
...
{	
	...
	{  
		...
		context->offsetX = newXRounded;
		if (changed) context->isTouchChanged |= 1;		
	}	
}
...
static void HandleXTouchChange(Ft_Esd_TouchScrollLogic *context)
{
	...
	//ESD_print("(%f, %f, %f, %f, %f)\n", newX, context->lastX, context->dX, context->OldDx, context->aX);
	//ESD_print("lastOffsetX : %f\n", context->lastOffsetX);
	//ESD_print("dX : %f\n", context->dX);
	
	double newXRounded = (context->dX + context->lastOffsetX);
	//ESD_print("raw Offset X : %f\n", newXRounded);			
	...
	{
		context->offsetX = newXRounded;
		//ESD_print("Offset X : %f\n\n\n", context->offsetX);	
		if (dif > 0) context->isTouchChanged |= 1;
	}	
}
...
static void HandleYTouchChange(Ft_Esd_TouchScrollLogic *context)
{
	...
	//ESD_print("(%f, %f, %f, %f, %f)\n", newY, context->lastY, context->dY, context->OldDx, context->aY);
	//ESD_print("lastOffsetY : %f\n", context->lastOffsetY);
	//ESD_print("dY : %f\n", context->dY);
	double newYRounded = (context->dY + context->lastOffsetY);
	//ESD_print("raw Offset Y : %f\n", newYRounded);
